...
\begin{figure}[h!]
  ...
  \caption{SPS for the Airbus latency improvement project.}
  \label{tbl:decisions}
\end{figure}

\vspace{0.5em}
After sponsor discussions since OS11, we have moved Ground Segment from fixed assumptions to an architectural decision (AD-06) to better address end-to-end latency. This reframing clarified that latency is not solely a function of link bandwidth, but of how routing authority, ground dependency, and data abstraction are structurally organized within the system architecture. Additional feedback and discussion with our sponsor led us to update AD-04.1 (Uplink Comms). It was determined that Ka-Band would be incur too much latency in comparison to S/X-band, which prioritizes link reliability over data size for uplink. Further, our architectural decision set has been refined to better reflect the structural drivers required to satisfy our SPS. The most consequential update is elevating the data path (AD-02) to the architectural anchor, as it structurally determines where information is processed, routed, and abstracted before ground interactions. The following justifies how each of these architectural decisions satisfies our problem space:
\vspace{-0.75em}
\begin{itemize}[noitemsep]
\item Data Volume Reduction (AD-01, AD-03): These ADs satisfy the “low-latency” requirement by minimizing the number of bits transmitted, thereby bypassing bandwidth bottlenecks.
\item Network Optimization (AD-02, AD-05, AD-06): These ADs enable “bypassing terrestrial infrastructure limitations” by using an adaptive mesh and strategic ground entry points to find the fastest path to the end-user.
\item Persistent Decision Flow (AD 04.1-04.3): Defines the communication architecture that enables distributed routing and limits ground bottlenecks, directly supporting persistent, low-latency performance. 
\end{itemize}
\vspace{-0.75em}
The identified architectural decisions (AD-01 through AD-06) represent a bounded and mutually exclusive set of technical tradeoffs. Each row in the morphological matrix forces a structural commitment that determines how information flows through the system. No additional architectural lever meaningfully influences latency without falling into implementation detail. Therefore, this set is sufficient to bound the tradespace while avoiding premature technology commitment. To achieve low-latency, the architecture allows for trading on-board processing levels (AD-01, AD-03) to minimize data volume before it ever hits the downlink bottleneck. To bypass terrestrial limitations, the tradespace includes multiple networking and routing configurations (AD-02, AD-05) that enable the system to find the optimal path to the end-user, regardless of local ground station availability. Persistence and ground network architecture explicit decisions (AD-07, AD-06); this allows the team to optimize the geometric relationship between satellites and ground entry points to minimize "waiting time". Finally, by treating uplink communication (AD-04.1) as an open decision focused on S/X-Band reliability over raw speed, the architecture ensures the persistent availability of the command-and-control link under diverse atmospheric conditions. Collectively, these open decisions provide the necessary degrees of freedom to converge on a design that satisfies the rigorous latency and persistence requirements of the project. Therefore, at this stage, most decisions remain open as the team enters tradespace exploration. 
