...
\noindent Functions are grouped by temporal handoffs \textit{(e.g., what comes next?)}, making this view effective for understanding the end-to-end performance flow. The CONOP-based decomposition, therefore, provides strong insight into system behavior and execution sequencing, though architectural ownership boundaries remain less explicit.
...
\subsubsection{Reevaluation Approach}
Because of the DSM's size and density, manual inspection alone made it difficult to identify clustering patterns, particularly when influenced by the Initial Level I perspective. The team therefore leveraged DSM analysis tooling to support clustering and visualization \cite{dsm_editor_2024}.
