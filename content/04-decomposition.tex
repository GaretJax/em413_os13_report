...
Functions are grouped according to temporal handoffs \textit{(e.g., what comes next?)}, making this view effective for understanding end-to-end performance flow. The CONOP-based decomposition therefore provides strong insight into system behavior and execution sequencing, though architectural ownership boundaries remain less explicit.
\todo{Need to add refs.}

Level II decompositions were derived from this structure (full Initial Level I DSM provided in Appendix NEED REF). Because Level I followed the CONOP, the operational perspective influenced not only grouping but also how Level II functions were defined and named. Functions framed around mission phases emphasized sequential behavior over persistent system responsibilities, shaping interpretation of relationships and how dependencies were represented within the DSM.

\subsubsection{Reevaluation Approach}
\todo{Add Red to "DSM Editor"}
Because of the size and density of the DSM, manual inspection alone made it difficult to identify coupling patterns, particularly when influenced by the Initial Level I perspective. The team therefore leveraged DSM analysis tooling to support clustering and visualization. Automated clustering enabled clearer observation of tightly coupled functional groupings and supported comparison between the CONOP-based decomposition and alternative architectural partitionings.

\subsubsection{Revised Level I Decomposition}
The revised Level-1 decomposition reorganizes functions according to persistent system responsibilities rather than operational sequence, providing a different interpretation of the DSM. Under this structure, previously dispersed Level-2 functions form clearer clusters, revealing strong coupling between data management, onboard processing, and communications functions that was less visible in the CONOP-based view. This suggests many dependencies arise from shared resources and continuous coordination rather than sequential handoffs. The revised decomposition therefore aligns more closely with observed DSM coupling and exposes architectural boundaries that remain stable across operational scenarios. While the CONOP view remains effective for understanding mission execution and latency flow, the responsibility-based decomposition is preferred because it better reflects subsystem ownership, interface definition, and the system’s underlying architectural organization.
